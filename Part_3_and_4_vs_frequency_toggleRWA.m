...
T = 10;                       % Time duration of the evolution, can adjust this to see the difference between fast and slow move
...
% A = 0.01;
wwidth = 4*pi/T;
% warray = [1.001 0.99979];
warray = linspace(1-wwidth,1+wwidth,100);
len_warray = 100;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
no_RWA_prob = zeros(1,length(warray));
for w_ind = 1:len_warray
    w = warray(w_ind);
    term_1 = (exp(1i*(1+ w)*T) -1)/(1+ w);
    term_2 = (exp(1i*(1-w)*T) -1)/(1-w);
    no_RWA_prob(w_ind) = abs( V_10 * (term_1 + term_2) )^2;
end
plot((1-warray)/(2*pi/T), no_RWA_prob, 'g')
hold on

% Plot numerical split operator evolution
...
plot((1-warray)/(2*pi/T),trans_prob,'r')
...
Amp
yline(Amp);
legend('Theory (RWA)','Theory (no RWA)','Numerical','$|\langle\psi_{1}^{0}|\frac{A}{2}sin(x)|\psi_{0}^{0}\rangle|^{2} T^{2}$','Interpreter','latex','FontSize',10)
