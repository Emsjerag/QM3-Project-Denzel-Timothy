...
correct_RWA_prob = zeros(1,M);
second_term = zeros(1,M);
for m = 1:M
...
%         end
        ...
        sum = sum + VnkVkm_array(k)*(-(term_1a+term_1b)*term_1mul + term_2);
        sum1 = sum1 + VnkVkm_array(k)*(-(term_1a)*term_1mul);
%         sum = sum + VnkVkm_array(k)*(term_2);
    end
    correct_RWA_prob(m) = abs(sum)^2;
    second_term(m) = abs(sum1)^2;
...
plot(t_grid, correct_RWA_prob, 'k')
plot(t_grid, second_term, 'g')


xlabel('$\frac{\omega_{20}-2\omega}{2\pi} t_{i}$','Interpreter','latex','FontSize',22)
ylabel('${P_{2\leftarrow 0}}$','Interpreter','latex','FontSize',22)
legend('Numerical','Theory (lecture RWA)', 'Theory (correct RWA)', 'Second Term Contributions', 'FontSize', 21)
% legend('Numerical')
title(['$\omega  = $ ', num2str(w)],'Interpreter','latex','FontSize',25)