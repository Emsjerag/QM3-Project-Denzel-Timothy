...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
t_grid = linspace(0,(2-1*w)/(2*pi)*T,M);
sin_arr = diag(A/2*sin(X - X0));
temp = sin_arr*VE_INI_g';
V_20 = dot(VE_INI_2e,temp);

theoreticalP = abs(V_20)^2*4*sin(t_grid*pi).^2/(1-w)^2;
...
xlabel('$\frac{\omega_{20}-\omega}{2\pi} t_{i}$','Interpreter','latex','FontSize',20)
